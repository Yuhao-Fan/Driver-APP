...
class ItemDivideComponent extends Component {
    render() {
      return (
        <View style={{height: 1, ackgroundColor: 'skyblue'}}/>
      );
    }
  };
export default class OrderList extends Component{
    ...
    fetchData(){
        ...
        //alert(token);
        fetch("https://www.kingdom174.work/HistoricalFeedback?token="+token,{method:'GET'})
      ...
      renderFeedback({ item }) {
        ...
        return (
            ...
            <TouchableOpacity onPress={() => alert('1st')}>
              <Text style={styles.Data}>标题：{item.Title}</Text>
              <Text style={styles.Data}>内容：{item.Content}</Text>
              <Text style={styles.Data}>时间：{item.Time}</Text>
              <Text style={styles.Data}>回复：{item.Answer}</Text>

...
import StartOrderView from './module/StartOrder'
...
export default class Home extends React.Component {
  ...
  constructor(props) {
    ...
    this.DriverMove = this.DriverMove.bind(this);
    this.RefreshDriverPosition = this.RefreshDriverPosition.bind(this);
  }
  ...
  Getcity(nativeEvent)//更新当前位置
  {
    ...
    this.setState({unlockVision:true})//解锁，防止跳回
  ...
  _logLocationEvent = ( nativeEvent ) => {
    ...
    // fetch("https://www.kingdom174.work/send_pos?token="+token+"&position="+"{"+Driverlatitude+","+Driverlongitude+"}"
    //     ,{ method: 'POST',
    //       // body:"token=ycKE8WaB5hjuh6b&position={113.65,23.16}"
    //       // mode: 'cors',
    //       // body:JSON.stringify({
    //       //   token: token,
    //       //   // position: "{"+Driverlongitude+","+Driverlatitude+"}",
    //       //   position:"{113.65,23.16}"
    //       // })
    //     })
    //     .then(response=>response.json())
    //     .then(responseJson=>{
    ...
    //!!!如果正常，应当取消注释，现在还是使用的模拟位置
    this.setState({Driverlatitude:nativeEvent.latitude,Driverlongitude:nativeEvent.longitude})
    if (this.state.monted)
      this.Getcity({nativeEvent})//改动
  }
  ...
  CheckMap= (Fromlatitude, Fromlongitide, Tolatitude, Tolongitude) =>//检查zoom是否合适
  {
    if(Fromlatitude&&Tolatitude)
    {

      // this.setState({findpath:true})
      //var distance=this.getGreatCircleDistance(0,0,100,100)
      console.log("checkmap")
      //this.setState({test:distance,zoom:19})
      //distance=1000000
      let mid_cord={latitude:( Fromlatitude+Tolatitude)/2,
        longitude: ( Fromlongitide+Tolongitude)/2}
      var distance=100
      //fan:报错原因不明
      this.mapView.setStatus({center: {
          latitude:( Fromlatitude+Tolatitude)/2,
          longitude: ( Fromlongitide+Tolongitude)/2
        }})
      if(distance<=500) { this.mapView.setStatus({zoomLevel:18})}
      else if(distance<=1000) {this.mapView.setStatus({zoomLevel:15})}
      else if(distance<=10000) {this.mapView.setStatus({zoomLevel:10})}
      else if(distance<=100000) {this.mapView.setStatus({zoomLevel:8})}
      else if(distance<=1000000) {this.mapView.setStatus({zoomLevel:7})}
      else if(distance<=10000000) {this.mapView.setStatus({zoomLevel:4})}
      else this.mapView.setStatus({zoomLevel:1})
    }
  }
  ...
  Route= (Fromlatitude, Fromlongitide, Tolatitude, Tolongitude) =>//得到导航路径点
  {
    ...
    if (this.state.Nowlatitude && this.state.Togolatitude) {
      //this.CheckMap(Fromlatitude, Fromlongitide, Tolatitude, Tolongitude)
      this._routeline = []
      var route_length = 0
      //let taxi_cost = 0;
      let NowLocation = {
        Nowlatitude: this.state.Nowlatitude,
        Nowlongitude: this.state.Nowlongitude
      }
      let TogoLocation = {
      ...
      //此处使用驾车导航api，还有步行公交骑行等
      fetch("https://restapi.amap.com/v3/direction/driving?key=4c87d15289eda08afc5384a4b90100c2&origin=" + Fromlongitide + "," + Fromlatitude + "&destination=" + Tolongitude + "," + Tolatitude + "&originid=&destinationid=&extensions=all&strategy=0&waypoints=&avoidpolygons=&avoidroad=")
        .then(response => response.json())
        .then(json => {
  ...
  _BottomSwitch= (mode) => {
    ...
    if (mode == 0) {
      ...
      this._clearRoute();
      this.setState({drivermode:0})
    }
    else {
      const { Driverlatitude,Driverlongitude,Nowlatitude, Nowlongitude, Togolatitude, Togolongitude } = this.state
      if (Togolatitude == '' || Togolongitude == '' || Nowlatitude == '' || Nowlongitude == '') {
        ...
        var { orderData } = this.state
        fetch("https://restapi.amap.com/v3/direction/driving?key=4c87d15289eda08afc5384a4b90100c2&origin=" + Nowlongitude + "," + Nowlatitude + "&destination=" + Togolongitude + "," + Togolatitude + "&originid=&destinationid=&extensions=all&strategy=0&waypoints=&avoidpolygons=&avoidroad=")
            .then(response => response.json())
            .then(json => {
              if (json.route.taxi_cost != null)
                orderData.taxi_cost = json.route.taxi_cost;
              console.log("taxi_cost",json.route.taxi_cost )
            })
        this.setState({drivermode: 1})//1:接到了新的订单
        console.log("drivermodel1")
        bottomHeight = orderViewHeight
        this.setState({unlockVision:true})
      this.mapView.setStatus(
        {
          tilt: 45,
          rotation: 90,
        ...
        this.Route(Driverlatitude,Driverlongitude,Nowlatitude, Nowlongitude)
        console.log("查看成功")
        //alert("查看成功")
  ...
  _orderProcess= (orderData) => {
    ...
    this.Route(this.state.Driverlatitude, this.state.Driverlongitude , this.state.Nowlatitude, this.state.Nowlongitude)
    //这里模拟间隔1s接受司机的位置
    this.carHandle = setInterval(() => {
    //while(this.state.drivermode==1){
      let posi = { latitude: this.state.Driverlatitude, longitude: this.state.Driverlongitude }
    ...
    // qp = 0;
    // this.carHandle = setInterval(() => {
    //   //let posi={latitude: Driverlatitude, longitude: Driverlongitude }
...
PosToNum(pos){
  let startpos=pos.split(",");
  let lat=startpos[0].substring(1);
  //console.log(lat.substring(1));
  let lon=startpos[1].split("}")[0];
  //console.log(lon.split("}")[0]);
  return [lat,lon];
}
  ...
  FindOrder() {
      ...
      this.mapView.setStatus(
          {
            ...
            // zoomLevel: 18,
            center: {
              latitude: this.state.Nowlatitude,
              longitude: this.state.Nowlongitude
            }
          },1000
      );
    }
    else
    {
      ...
      const{token,Driverlatitude,Driverlongitude}=this.state
      console.log(token,"{"+Driverlatitude+","+Driverlongitude+"}")
      var Now = {
      ...
      fetch("https://www.kingdom174.work/find_order?token="+token+"&position="+Driverlatitude+","+Driverlongitude
          ,{ method: 'POST',
        // body:"token=ycKE8WaB5hjuh6b&position={113.65,23.16}"
        // mode: 'cors',
        // body:JSON.stringify({
        ...
        // })
          ...
          .then(response=>response.json())
          .then(responseJson=>{
            //console.log(responseJson.Destination);
            this.setState({PassengerID:responseJson.PassengerID, OrderID:responseJson.OrderID})
            let now=this.PosToNum(responseJson.StartPlace)
            // Now.longitude= now[0]
            // Now.latitude=now[1]
            let to=this.PosToNum(responseJson.StartPlace)
            // To.nativeEvent.longitude= to[0]
            // To.nativeEvent.latitude=to[1]
            this.setState({PassengerID:responseJson.PasrsengerID,
              Togolongitude:To.nativeEvent.longitude,
              Togolatitude:To.nativeEvent.latitude,
              Nowlongitude:Now.longitude,
              Nowlatitude:Now.latitude,
              OrderID:responseJson.OrderID})
            //console.log(Now.longitude,"1")
  ...
  _cancelOrder() {
    this.setState({ orderMode: 0, bottommode: 0,drivermode:0,NowLocation:'',Togo:'' })
  this._clearRoute();
  this.setState({unlockVision:false})
  }
      ...
      <View style={styles.container}>
        ...
        >
          ...
          >
              ...
              <Image style={{ width: 58, height: 57 }} source={require('../images/destination.png')}/>

              <Text>乘客目的地</Text>
            </View>
          </MapView.Marker>
          <MapView.Marker active coordinate={Pos.Nowpos}>
            <View style={styles.defaultbox}>
              <Image style={{ width: 50, height: 58 }} source={require('../images/passenger2.png')}/>

              <Text>乘客位置</Text>
            </View>
          </MapView.Marker>
          <MapView.Marker active coordinate={driverPos}>

            <View style={styles.defaultbox}>
              <Image style={{ width: 38, height: 58 }} source={require('../images/driver.png')}/>
              <Text>我的位置</Text>
            </View>
          </MapView.Marker>
          ...
          {this.state.orderMode == 0 &&
            <View style={{ flex: 1 }}>
              <View style={sty.BotTop}>
                <TouchableOpacity style={sty.Bottom1} onPress={() => { this._BottomSwitch(0) }}><Text style={sty.fontSize}>返回</Text></TouchableOpacity>
                <TouchableOpacity style={sty.Bottom1} onPress={() => { this._BottomSwitch(1) }}>
                  {this.state.drivermode==0?
                      <Text style={sty.fontSize}>查详情</Text>
                      :<Text style={sty.fontSize}>到乘客</Text>
                  }

                </TouchableOpacity>
                <TouchableOpacity style={sty.Bottom1} onPress={() => { this.FindOrder() }}>
                  {this.state.drivermode==0?
                      <Text style={sty.fontSize}>接新单</Text>
                      :<Text style={sty.fontSize}>到终点</Text>
                  }
                </TouchableOpacity>
